Newspeak output
---------------
503.c
void main(void) {
  int32 tmp___0;
  int32 tmp___1;
  (503.c:29#1094)^choose {
   -->
    (503.c:29#1094)^guard(! (a_int32 ==_int32 0));
    (503.c:29#1094)^tmp___1 =(int32) b_int32;
   -->
    (503.c:29#1094)^guard((a_int32 ==_int32 0));
    (503.c:29#1094)^tmp___1 =(int32) c_int32;
  }
  (503.c:29#1094)^choose {
   -->
    (503.c:29#1094)^guard(! (tmp___1_int32 ==_int32 0));
    (503.c:29#1094)^tmp___0 =(int32) d_int32;
   -->
    (503.c:29#1094)^guard((tmp___1_int32 ==_int32 0));
    (503.c:29#1094)^tmp___0 =(int32) e_int32;
  }
  (503.c:29#1094)^choose {
   -->
    (503.c:29#1094)^guard(! (tmp___0_int32 ==_int32 0));
    (503.c:30#1117)^x =(int32) 0;
    (503.c:31#1128)^x =(int32) 1;
    (503.c:32#1139)^x =(int32) 2;
    (503.c:33#1150)^x =(int32) 3;
    (503.c:34#1161)^x =(int32) 4;
   -->
    (503.c:29#1094)^guard((tmp___0_int32 ==_int32 0));
    (503.c:36#1183)^x =(int32) 0;
    (503.c:37#1194)^x =(int32) 1;
    (503.c:38#1205)^x =(int32) 2;
    (503.c:39#1216)^x =(int32) 3;
    (503.c:40#1227)^x =(int32) 4;
    (503.c:41#1238)^x =(int32) 5;
  }
}

int32 a;
int32 b;
int32 c;
int32 d;
int32 e;
int32 x;

