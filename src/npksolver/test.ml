...
let run _ =
  ...
  let ae got exp name =
    assert_equal ~printer:(to_string dom) got exp name
  in
  ...
  let b = Box.set_var dom (Prog.G "x") (Range.from_bounds 5 5) top in
  ...
  let b = Box.set_var dom (Prog.L 0) (Range.from_bounds 5 5) top in
