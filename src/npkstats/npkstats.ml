...
let speclist = 
  [("--count-call fid", Arg.String add_counted_call, 
   ...
   ("--funstats", Arg.Set arg_funstats, "more statistics on functions");
...
let process input =
  ...
  let callstats = Hashtbl.create 10 in
  let globals = ref 0 in
  let bytes = ref 0 in
  let funstats = Hashtbl.create 10 in
  let globstats = Hashtbl.create 10 in
  let void_fun = ref 0 in
  let counters = init_counters () in
  let count_call f =
    try
  ...
  let gen_graphs fname =
    ...
    in
      ...
      close_out cout
  in
  let current_counters = init_counters () in
  let collector =
    ...
    ; func_after = (fun () ->
         ...
         reinit_counters current_counters)
    ; gdecl = (fun _ t ->
         globals := !globals + 1;
         if !more_verb then 
           try 
         ...
         incr_bytes ((N.size_of prog.ptr_sz t) / 8);
         true)
    }
  in
  List.iter (fun f -> Hashtbl.add callstats f 0) !fun_to_count;

  let max_stats = Maxcount.count !debug prog in
    Lowspeak.visit collector prog;
    let cout = 
      if !xout <> "" then begin
    ...
    in
      print_endline (to_string !verbose cout);
      ...
      if !graphs then gen_graphs !output;
      if !arg_funstats then Funstats.collect prog
